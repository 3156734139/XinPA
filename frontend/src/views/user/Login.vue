    ...
    <div class="login-wrapper">
        ...
        <div class="brand-content">
          ...
          <div class="brand-features">
            <div class="feature-item">
              ...
              <span>订单管理 · 客户档案 · 财务记账</span>
            </div>
            <div class="feature-item">
              <span class="feature-dot"></span>
              <span>素材库 · 预约日历 · 价目套餐</span>
            </div>
            <div class="feature-item">
              <span class="feature-dot"></span>
              <span>AI 助理 · 数据统计 · 待办看板</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="form-section">
        <div class="form-card">
          ...
          <!-- 验证码登录 -->
          <form v-show="activeTab === 'sms'" @submit.prevent="handleSmsLogin" class="login-form">
            ...
            <div class="method-switch">
              <button type="button" class="switch-link" @click="switchTab('password')">使用密码登录</button>
            </div>
          </form>

          <!-- 密码登录 -->
          <form v-show="activeTab === 'password'" @submit.prevent="handlePasswordLogin" class="login-form">
            <el-input
            ...
            <div class="method-switch">
              <button type="button" class="switch-link" @click="switchTab('sms')">使用验证码登录</button>
            </div>
          </form>
...
async function handleSendCode() {
  ...
  try {
    const res: any = await sendSmsCode(phone.value);
    ...
    showError(e?.response?.data?.message || '发送失败，请稍后重试');
  } finally { codeSending.value = false; }
}

async function handleSmsLogin() {
  if (!smsForm.code || !phone.value) { showError('请填写手机号和验证码'); return; }
  if (!/^1\d{10}$/.test(phone.value)) { showError('手机号格式不正确'); return; }
  loading.value = true; errorMsg.value = '';
  try {
    const res: any = await login({ phone: phone.value, code: smsForm.code, loginType: 'sms' });
    if (res.code !== 200) { showError(res.message || '登录失败'); return; }
  ...
  } catch (e: any) { showError(e?.response?.data?.message || '登录失败，请检查网络连接'); }
  finally { loading.value = false; }
}

async function handlePasswordLogin() {
  if (!pwdForm.password || !phone.value) { showError('请填写手机号和密码'); return; }
  if (!/^1\d{10}$/.test(phone.value)) { showError('手机号格式不正确'); return; }
  loading.value = true; errorMsg.value = '';
  try {
    const res: any = await login({ phone: phone.value, password: pwdForm.password, loginType: 'password' });
    if (res.code !== 200) { showError(res.message || '登录失败'); return; }
    userStore.setToken(res.data.token);
    if (res.data.refreshToken) userStore.setRefreshToken(res.data.refreshToken);
    userStore.setUserType('USER');
...
.bg-shape-3 {
  ...
  background: radial-gradient(circle, rgba(232, 120, 154, 0.08), transparent);
  top: 40%; left: 30%;
}
